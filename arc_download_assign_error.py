...
def execute(name, is_this_week, csv_lines, is_first_arc_number):
    ...
    try:
        ...
        for arc_number in arc_numbers:
            ...
            create_list_html = arc_model.create_list(token, ped, action, arc_number, selected_status_id=selected_status_id)
            ...
            csv_text = arc_model.get_csv(name, is_this_week, ped, action, arc_number, token, from_date, to_date,
                              selected_status_id=selected_status_id, transaction_type="SA", form_of_payment="CA", tick=tick)
...
if csv_lines:
    ...
    mail_subject = 'IAR DOWNLOAD E-SA-CA'
    try:
        body = "\r\n".join(csv_lines)
        floder = "csv"
        file_name = "all_E_SA_CA.csv"
        arc_model.csv_write(floder, file_name, body)
        mail = arc.Email(smtp_server=mail_smtp_server)
        mail.send(mail_from_addr, mail_to_addr, mail_subject, "Please check the attachment.",
                  [floder + "/" + file_name])
